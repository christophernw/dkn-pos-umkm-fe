 ...
 */
export const generateDebtReportPDF = async (
  ...
  const addPageFooter = (currentPage: number, totalPages: number): void => {
    ...
    const generatedDate: string = `Laporan dibuat pada ${formatDateWithTime(
    ...
    pdf.text(generatedDate, pageWidth / 2, pageHeight - 15, {
      align: "center",
    });
    pdf.text(
    ...
    headerContainer.innerHTML = `
      <div style="padding: 40px; font-family: Arial, sans-serif;">
        <div style="text-align: center; margin-bottom: 30px;">
          ...
          <p style="color: #666;">
            Periode: ${formatDate(reportData.startDate)} - ${formatDate(
      reportData.endDate
    )}
          </p>
        </div>
              ...
              <tbody>
                ...
                `
                  ...
                  <tr>
                    ...
                    <td style="padding: 12px; text-align: right; font-weight: 700; ${getBalanceColor(
                      reportData.totalPemasukan,
                      reportData.totalPengeluaran
                    )}">
                      Rp ${formatCurrency(
                        Math.abs(
                          (reportData.totalPemasukan ?? 0) -
  ...
  const createTableHeader = async ({
    yPosition,
  }: TableHeaderOptions): Promise<number> => {
  ...
  const createTransactionRows = async (
    ...
    if (maxHeight) {
      ...
      const allTransactionsHeight = await measureTransactionRowsHeight(
        transactionsToRender
      );

      // If they fit, render them all
      if (allTransactionsHeight <= maxHeight) {
        rowsHeight = await renderTransactionRows(
          transactionsToRender,
          yPosition
        );
  ...
  const measureTransactionRowsHeight = async (
    transactions: any[]
  ): Promise<number> => {
  ...
  const renderTransactionRows = async (
    transactions: any[],
    yPosition: number
  ): Promise<number> => {
  ...
  const generatePDF = async () => {
    ...
    const estimatedRemainingPages = Math.ceil(
      remainingTransactions / ROWS_PER_PAGE
    );
    ...
    while (transactionIndex < totalTransactions) {
      ...
      const remainingTransactions =
        reportData.transactions.slice(transactionIndex);

      // Render as many transactions as possible on this page
      const { renderedTransactions: transactionsRendered } =
        await createTransactionRows(
...
// Add this helper function at the bottom of the file, before the last export
const getBalanceColor = (pemasukan?: number, pengeluaran?: number): string => {
...
// Add this helper function at the bottom of the file, before the last export
const getTransactionType = (isArusKas: boolean, transaction: any): string => {
  if (isArusKas) {
    return transaction.jenis === "inflow" ? "Kas Masuk" : "Kas Keluar";
  }
  return transaction.transaction_type === "pemasukan" ? "Berikan" : "Terima";
};

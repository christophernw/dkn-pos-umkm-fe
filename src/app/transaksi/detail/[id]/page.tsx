...
export default function TransaksiDetailPage() {
  ...
  useEffect(() => {
    if (!accessToken || user?.is_bpr) return; // Add check to prevent API calls for BPR users
  ...
  }, [transactionId, accessToken, showModal, user?.is_bpr]); // Add user?.is_bpr to dependencies
  ...
  // Check if user is BPR - Move this check to the beginning of the component
  if (user?.is_bpr) {

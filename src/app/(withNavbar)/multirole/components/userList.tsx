...
const UserList = () => {
  ...
  const confirmDeleteUser = async (userToRemove: User) => {
    try {
      setIsDeleting(true);
      setError(null);
      ...
      setError(error.message);
      showModal("Gagal", `Gagal menghapus pengguna: ${error.message}`, "error");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleDeleteUser = (userToRemove: User) => {
    showModal(
      ...
      {
        label: "Hapus",
        onClick: () => confirmDeleteUser(userToRemove), // tidak async
      },
      {
        label: "Batal",
        onClick: () => hideModal(),
  ...
  const confirmDeleteInvitation = async (invitationId: number) => {
    try {
      setIsDeleting(true);
      setError(null);
      ...
      setError(error.message);
      showModal("Gagal", `Gagal menghapus undangan: ${error.message}`, "error");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleDeleteInvitation = (invitationId: number) => {
    showModal(
      ...
      {
        label: "Hapus",
        onClick: () => confirmDeleteInvitation(invitationId), // tidak async
      },
      {
        label: "Batal",
        onClick: () => hideModal(),
  ...
  const pendingInvitationContent =
                ...
                style={{
                  backgroundColor:
                    invitation.role === "Pemilik"
                      ...
                      ? "#FFC107"
                      : "#3B82F6",
                  color: invitation.role === "Pengelola" ? "#000" : "#fff",
                }}
              >
                {invitation.role}
              </span>

...
const SemuaBarangPage: React.FC = () => {
  ...
  const fetchTopSellingProducts = async () => {
    ...
    try {
      const response = await fetch(
        `${config.apiUrl}/produk/top-selling/${selectedYear}/${selectedMonth}`,
        {
          method: "GET",
          headers: {
      ...
      setErrors((prev) => ({
        ...prev,
        topSelling: "Failed to load top selling products",
      }));
      setTopSellingProducts([]);
    } finally {
  ...
  const fetchLowStockProducts = async () => {
    ...
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        lowStock: "Failed to load low stock products",
      }));
      setLowStockProducts([]);
    } finally {
            ...
            <div className="space-y-4">
              {loading.topSelling ? (
                ...
                topSellingProducts.map((produk, index) => {
                  const percentage =
                    maxSold > 0 ? (produk.sold / maxSold) * 100 : 0;
                  const color =
                        ...
                        >
                          <div className="flex-shrink-0 mr-2">
                            <Image
                              src={
                                produk.imageUrl
                                  ? `${config.apiUrl}/media/${produk.imageUrl}`
                    ...
                    <div className="relative w-full aspect-square p-2">
                      <div className="relative w-full h-full overflow-hidden rounded-xl">
                        <Image
                          src={
                            produk.imageUrl
                              ? `${config.apiUrl}${produk.imageUrl.slice(4)}`
                              : `/images/placeholder.svg`
                          }
                          alt={produk.name}
                          fill
            ...
            {loading.lowStock ? (
              ...
              <div className="space-y-3">
                {lowStockProducts.map((produk) => (
                  <div
                    key={produk.id}
                    className="bg-white p-3 rounded-xl shadow flex items-center space-x-3"
                  >
